...
void query_exits_loop(void){
	...
	do {
		...
		res = query_exit(&ep);
		if (res < 0){
			if (verbose){ printf("TST: query exit failed with %d.\n", res);}
			return;
		}

		count= res;
		if (verbose){
			printf("TST: proc %d exited (remaining count: %d).\n", ep, count);
		}
		proc_exited(ep);
	} while (count > 0);
}
...
int do_retrieve(message * m){
	...
	if (verbose){
		printf("TST: Registering watch_exit for %d.\n", m->m_source);
	}
	if (watch_exit(m->m_source)!= OK){
		if (verbose) printf("TST: watchexit failed with errno: %d,\n", errno);
		return errno;
	};
...
int do_retrieve_nb(message * m){
	...
	if ( (storer != NOSERV) 	// equal when called from proc_exited
		...
		if (verbose && result != OK){
			printf("TST: cancelwatch for %d failed with %d.\n",  storer, errno);
		}
	}
	storer = NOSERV;
	...
	if (dequeue(&storers, &response) == OK){
		...
		if ((result = sendnb(response.m_source, &response)) != OK){
			printf("TST: revive send to %d failed with %d.\n", response.m_source, result);
		}
	}
...
static int do_store_common(endpoint_t source, int data, int start_watching){
	...
	storer = source;
	if (start_watching){
		if (verbose) printf("TST: Registering watch_exit for %d.\n", source);
  		if (watch_exit(storer)!= OK){
			printf("TST: watchexit failed with errno: %d,\n", errno);
			return -errno;
		}
	};
	// wake retriever
	...
	if (dequeue(&retrievers, &response)==OK){
		...
		if(verbose && result != OK){
			printf("TST: error for retrieve: %d\n", result);
		}
		if ((result = sendnb(response.m_source, &response)) != OK){
			printf("TST: retrieve answer send error %d.\n", result);
		}
	}
	return OK;
}
